...
const asset = path.join(root, "dist", "mathscinet-bibtex-exporter-v0.2.0.zip");

test("release package is installable from its root and matches the manifest version", () => {
  execFileSync(packageScript, ["0.2.0"], { cwd: root, stdio: "pipe" });

  const files = execFileSync("unzip", ["-Z1", asset], { encoding: "utf8" })
    .trim()
    .split("\n")
    .sort();
  assert.deepEqual(files, ["CHANGELOG.md", "LICENSE", "README.md", "content.js", "lib.js", "manifest.json"]);

  const archivedManifest = JSON.parse(execFileSync("unzip", ["-p", asset, "manifest.json"], { encoding: "utf8" }));
  assert.equal(archivedManifest.version, "0.2.0");
